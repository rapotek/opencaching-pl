...
$user_founds = XDb::xMultiVariableQueryValue(
    'SELECT `founds_count` FROM `user` WHERE `user_id`=:1 LIMIT 1',
    0,
    $user->getUserId()
);
...
if (isset($_POST['submitform']) && ($all_ok == true)) {
    if (isset($_POST['r']) && $_POST['r'] >= -3 && $_POST['r'] <= 3 && $mark_as_rated == true) {
        //mark_as_rated to avoid possibility to set rate to 0,1,2
    ...
    if (
        ...
        $is_any_cache_movedlog = XDb::xMultiVariableQueryValue(
            'SELECT COUNT(*) FROM `cache_moved` WHERE `cache_id` = :1 ',
            0,
            $geoCache->getCacheId()
        );
    ...
    $cache_status = XDb::xMultiVariableQueryValue(
        'SELECT `log_types`.`cache_status` FROM `log_types` WHERE `id`= :1 LIMIT 1',
        0,
        $log_type
    );
    ...
    if ($log_type == GeoCacheLog::LOGTYPE_FOUNDIT
        ...
        if ($top_cache == 1) {
            XDb::xSql(
                'INSERT IGNORE INTO `cache_rating` (`user_id`, `cache_id`) VALUES(?, ?)',
                $user->getUserId(),
                $geoCache->getCacheId()
            );
        } else {
            XDb::xSql(
                'DELETE FROM `cache_rating` WHERE `user_id`=? AND `cache_id`=?',
                $user->getUserId(),
                $geoCache->getCacheId()
            );
        }
...
foreach (GeoCacheLogCommons::logTypesArray() as $type) {
    ...
    if ($geoCache->getCacheType() != GeoCache::TYPE_EVENT
        ...
        // service log by Łza
        // if currently logged user is a cache owner and cache status is "available"
        ...
        // if currently logged user is a cache owner and cache status is "temp. unavailable"
        // then add log type option "available"
        // 10 = Ready to find
        if ($user->getUserId() == $geoCache->getOwnerId()
    ...
    if ($geoCache->getCacheType() == GeoCache::TYPE_EVENT) {
        ...
        $now->setTime(0, 0); //there is no time in $geoCache->getDatePlaced(),
    ...
    if ($type == $log_type) {
        $logtypeoptions .= '<option value="' . $type . '" selected="selected">'
            . htmlspecialchars(tr(GeoCacheLogCommons::typeTranslationKey($type)), ENT_COMPAT) . '</option>' . "\n";
    } else {
...
//set tpl vars
tpl_set_var('cachename', htmlspecialchars($geoCache->getCacheName(), ENT_COMPAT));
tpl_set_var('cacheid', htmlspecialchars($geoCache->getCacheId(), ENT_COMPAT));
tpl_set_var('logmin', htmlspecialchars($log_date_min, ENT_COMPAT));
tpl_set_var('loghour', htmlspecialchars($log_date_hour, ENT_COMPAT));
tpl_set_var('logday', htmlspecialchars($log_date_day, ENT_COMPAT));
tpl_set_var('logmonth', htmlspecialchars($log_date_month, ENT_COMPAT));
tpl_set_var('logyear', htmlspecialchars($log_date_year, ENT_COMPAT));
...
tpl_set_var('logtext', htmlspecialchars($log_text, ENT_COMPAT));
